  ...
  async execute(consent) {
    ...
    try {
      ...
      if (process.env.node_env === 'development') {
        if (this.ctx.userDefined.globalConfig.nhsNumberMapping[`${ patientId }`]) {
          patientId = this.ctx.userDefined.globalConfig.nhsNumberMapping[`${ patientId }`];
        }
      }

      logger.info('commands/postPatientConsent|policies-consenting-to', consent.length);
