...
module.exports = (adapter) => {
  return {
    ...
    exists: (patientUuid) => {
      ...
      const key = ['Fhir', ResourceName.PATIENT, 'by_uuid', patientUuid];
    ...
    set: (patientUuid, patient) => {
      ...
      const key = ['Fhir', ResourceName.PATIENT, 'by_uuid', patientUuid, 'data'];
    ...
    get: (patientUuid) => {
      ...
      const key = ['Fhir', ResourceName.PATIENT, 'by_uuid', patientUuid, 'data']; //@TODO Check why this data appears , its important !!!
    ...
    setNhsNumber: (patientUuid, nhsNumber) => {
      ...
      const key = ['Fhir', ResourceName.PATIENT, 'by_uuid', patientUuid, 'nhsNumber', nhsNumber];
    ...
    deleteAll: () => {
      ...
      const key = ['Fhir', ResourceName.PATIENT, 'by_uuid'];
      adapter.delete(key);
    },
    ...
    getPractitionerUuid: (patientUuid) => {
      ...
      const key = ['Fhir', ResourceName.PATIENT, 'by_uuid', patientUuid, 'practitioner'];

      return adapter.get(key);
    },

    /**
     * Exists practitioner uuid for patient
     *
     * @param  {string} patientUuid
     * @return {string}
     */
    existsPractitionerUuid: (patientUuid) => {
      logger.info('mixins/patient|byPatientUuid|existsPractitionerUuid', { patientUuid });
    ...
    getByPatientUuids: (patientUuids) => {
      ...
      const patients = patientUuids.map((patientUuid) => {
        return adapter.getObjectWithArrays(['Fhir', ResourceName.PATIENT, 'by_uuid', patientUuid]);
      });
