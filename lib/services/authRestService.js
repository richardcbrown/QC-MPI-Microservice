...
class AuthRestService {
  ...
  static create(ctx) {
    ...
    return new AuthRestService(ctx, ctx.globalConfig);
  }
  ...
  async authenticate() {
    ...
    const options = {
      ...
      headers: {
        'authorization': `Basic ${ Buffer.from(this.hostConfig.auth.client_id + ':' + this.hostConfig.auth.client_secret).toString('base64') }`
      },
      json: true
    };
