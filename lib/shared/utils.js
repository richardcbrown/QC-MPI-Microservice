...
function getPractitionerRef(resource) {
  ...
  if (resource.resourceType === ResourceName.PATIENT) {
    ...
    if (resource.generalPractitioner) {
      ...
      const gpReference = resource.generalPractitioner.find((gp) => gp.reference && gp.reference.startsWith(ResourceName.ORGANIZATION));

      practitionerReference = gpReference && gpReference.reference || null;
    }
  }
