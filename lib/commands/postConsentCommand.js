...
class PostPatientConsentCommand {
   ...
   */
  async execute(consentingPolicies) {
    ...
    try {
      ...
      logger.info('commands/postPatientConsent|policies-consenting-to', consentingPolicies.length);
      ...
      const apiHost = this.ctx.userDefined.globalConfig.api.host

      consentingPolicies.forEach((c) => {
        consents.push({
          resourceType: 'Consent',
          policy: [{ uri: `${apiHost}/Policy/${ c.id }`}],
          status: 'active',
          patient: {
      ...
      logger.info('commands/postPatientConsent|sending-consents', consents.length);

      await P.each(consents, async (c) => {
        await resourceService.postResource('Consent', c);
      });

      logger.info('commands/postPatientConsent|consents-sent', consents.length);
