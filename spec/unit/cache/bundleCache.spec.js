...
describe('ripple-cdr-lib/lib/cache/patientBundleCache', () => {
  ...
  function seeds() {
    qewdSession.data.$(['Fhir', 'PatientBundle', 'by_nhsNumber', nhsNumber, 'Patient']).setDocument({
    ...
    qewdSession.data.$(['Fhir', 'PatientBundle', 'by_uuid']).setDocument({
      'c57c65f2-1ca8-46df-9a29-09373dcff552': {
        value: 'foo',
        testArray: [1, 2]
      },
  ...
  describe('#import', () => {
    it('should set data to bundle cache', () => {
      ...
      const actual = qewdSession.data.$(['Fhir', 'PatientBundle']).getDocument(true);
      expect(actual).toEqual(expected);
    });
  });
