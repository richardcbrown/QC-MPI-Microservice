...
const reformatTelephoneNumber = require('../formatters/reformatTelephoneNumber');
...
class GetDemographicsCommand {
  ...
  async execute(patientId) {
    ...
    try {
      ...
      if (cachedObj) {
        return reformatTelephoneNumber(cachedObj);
      }
      ...
      let responseObj = demographicService.getByPatientId(patientId);
      debug('response: %j', responseObj);

      return reformatTelephoneNumber(responseObj);
    } catch (error) {
